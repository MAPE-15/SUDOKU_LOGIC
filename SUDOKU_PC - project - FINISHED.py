# SUDOKU_PC, pc will do it itself
# FINISHED !!!
# but it's random based, so it won't learn itself how to complete, it's just on serendipity
...
def sudoku():
    ...
    while ide:
        ...
        # you cannot sign the different number, cannot change the number in array_row_columns array
        # if str(number) in array_rows_columns[rows.index(random_row_column[0]), columns.index(random_row_column[1])]:
            # pass

        # you cannot sign the different number, cannot change the number in array_blocks array
        # elif str(number) in array_blocks[random_block, random_block_row, random_block_column]:
            # pass
        ...
        # if the board does not contain any 'x' in it, end the process
        if 'x' not in array_rows_columns[:, :]:
            print('')
            print("""PC HAS COMPLETED THE SUDOKU
THANK YOU FOR TRYING MY SUDOKU PC, BYE...""")
